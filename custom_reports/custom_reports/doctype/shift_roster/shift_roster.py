...
def mark_attendance(shift_roster):
	...
	if ros_days:
		for rsday in ros_days:
			...
			if rsday.day_type in ['OW','AL','UL','A','P','SL','RO','CO.L']:
				Attendance=frappe.db.get_value('Attendance',{'employee':rsday.employee,'attendance_date':rsday.day,'docstatus':'1'},'name')
				if Attendance:
...
def employee_list(department,date_from,date_to):
	emp=frappe.db.get_all('Employee',filters={'department': department},fields=['name','employee_name','default_shift','weekly_off','weekly_off_2','holiday_list'],order_by='date_of_joining')
