...
class ShiftRoster(Document):
	def validate(self):		
		rs=frappe.db.sql("""select * from `tabShift Roster` where department='{0}' and ('{1}' BETWEEN date_from AND date_to OR '{2}' BETWEEN date_from AND date_to OR ('{1}' >= date_from AND '{2}' <= date_to )) and name!='{3}' """.format(self.department,self.date_from,self.date_to,self.name),as_dict=1,debug=0)
		if rs:
