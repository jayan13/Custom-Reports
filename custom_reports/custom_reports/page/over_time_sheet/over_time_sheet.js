...
frappe.pages['over-time-sheet'].on_page_load = function(wrapper) {
	new MyPage(wrapper);
		...
		MyPage =Class.extend({
			...
			make: function()
			{
				...
				frappe.call({
					method: 'custom_reports.custom_reports.page.over_time_sheet.over_time_sheet.get_report',
					freeze: 1,
					...
					callback: function (r) {
					  if (r.message) {							
							$(frappe.render_template("over_time_sheet",r.message)).appendTo(wrp);	
					  }
					},
				  });
