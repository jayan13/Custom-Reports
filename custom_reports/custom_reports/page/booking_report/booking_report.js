...
frappe.pages['booking-report'].on_page_load = function(wrapper) {
	new MyPage(wrapper);
}
		...
		MyPage =Class.extend({
			...
			make: function()
			{
				...
				let data='';
				$(frappe.render_template("booking_report",data)).appendTo(this.page.main);
				...
				function get_report()
				{
					...
					{		
							frappe.call({
							method: 'custom_reports.custom_reports.page.booking_report.booking_report.get_report',
							freeze: 1,
							...
							callback: function (r) {
							  if (r.message) {
								  $('#report_egg').html('');
									...
									let data2=r.message;									
									$(frappe.render_template("booking_report_data",data2)).appendTo("#report_egg");
							  }
							},
						  });
