...
def get_report(customer=None,from_date=None,to_date=None):
    ...
    data['items']=frappe.db.sql("""select ad.address_title,sum(si.total_qty) as total_qty,sum(si.total) as total,sum(si.total_taxes_and_charges) as total_taxes,sum(si.grand_total) as  grand_total from `tabSales Invoice` si left join `tabAddress` ad on ad.name=si.customer_address where si.company='Quick Laundry – Sole Proprietorship LLC' and ad.address_title in ('F&B LINEN', 'GUEST LAUNDRY', 'ROOM LINEN', 'RECREATION LINEN', 'STAFF UNIFORM', 'MANAGEMENT LAUNDRY') and si.docstatus=1 and si.customer='{0}' and (si.posting_date between '{1}' and '{2}') group by ad.address_title """.format(customer,from_date,to_date),as_dict=1,debug=0)
