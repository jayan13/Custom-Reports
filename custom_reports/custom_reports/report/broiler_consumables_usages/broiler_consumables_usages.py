...
def get_data(conditions,filters):
	...
	if broiler_batchs:
		for broilerbatch in broiler_batchs:
			#if broilerbatch.broiler_batch!='626-boriler-herz-28-shed-1':
			#	continue
			vac=0
			med=0
			starter=0
			...
			strt=frappe.db.sql("""select DISTINCT starter_item as item from `tabFeed` where parent='{0}' """.format(broiler_batch),as_dict=1,debug=0)
			if strt:
				for md in strt:
					stk_qty=0
					stock_ent_med=frappe.db.sql(""" select sum(qty) as qty,sum(amount) as amount,posting_date from `tabStock Entry Detail` left join `tabStock Entry` on `tabStock Entry`.name=`tabStock Entry Detail`.parent where `tabStock Entry`.project='{0}' 
					...
					starters=frappe.db.sql("""select b.name,b.broiler_shed,m.starter_item,sum(m.starter_qty) as qty,m.starter_uom,m.date,TIME(m.creation) as itime from `tabBroiler Batch` b left join `tabFeed`
						m on b.name=m.parent where m.starter_item!='' and b.company='{0}' and m.date >= '{1}' and m.date <= '{2}' and b.name='{3}' and m.starter_item='{4}' group by m.starter_item""".format(company,date_from,date_to,broiler_batch,md.item),as_dict=1,debug=0)
					if starters:
			...
			finsh=frappe.db.sql("""select DISTINCT finisher_item as item from `tabFeed` where parent='{0}' """.format(broiler_batch),as_dict=1,debug=0)
			if finsh:
				for md in finsh:
					stk_qty=0
					stock_ent_med=frappe.db.sql(""" select sum(qty) as qty,sum(amount) as amount,posting_date from `tabStock Entry Detail` left join `tabStock Entry` on `tabStock Entry`.name=`tabStock Entry Detail`.parent where `tabStock Entry`.project='{0}' 
					...
					finishers=frappe.db.sql("""select b.name,b.broiler_shed,m.finisher_item,sum(m.finisher_qty) as qty,m.finisher_uom,m.date,TIME(m.creation) as itime from `tabBroiler Batch` b left join `tabFeed`
						m on b.name=m.parent where  m.finisher_item!='' and b.company='{0}' and m.date >= '{1}' and m.date <= '{2}' and b.name='{3}' and m.finisher_item='{4}' group by m.finisher_item""".format(company,date_from,date_to,broiler_batch,md.item),as_dict=1,debug=0)
					if finishers:
