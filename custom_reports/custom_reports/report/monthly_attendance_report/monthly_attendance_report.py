...
from frappe.utils import cint, cstr, getdate, date_diff, add_to_date
...
def add_data(
	...
	for emp in employee_map:
		...
		if filters.group_by:
			row += [" "]
		row += [emp, emp_det.attendance_device_id,emp_det.employee_name]
...
def get_columns(filters):
	...
	columns += [_("Employee") + ":Link/Employee:120", _("Emp Code") + ":Data/:100", _("Employee Name") + ":Data/:120"]
	days = []
	date=add_to_date(filters.date_from,days=-1)
	for day in range(filters["total_days_in_month"]):
		date = add_to_date(date,days=1)
		day_name = day_abbr[getdate(str(date)).weekday()]		
		days.append(cstr(str(date).split('-')[2]) + " " + day_name + "::65")
	if not filters.summarized_view:
		columns += days
...
def get_conditions(filters):
	if not (filters.get("date_from") and filters.get("date_to")):
		msgprint(_("Please select Date from and tp"), raise_exception=1)

	filters["total_days_in_month"] = date_diff(filters.get("date_to"),filters.get("date_from"))+1

	conditions = " and attendance_date >= %(date_from)s and attendance_date <= %(date_to)s"
...
def get_employee_details(group_by, company):
	...
	query = """select name, employee_name, designation, department, branch, company,
		holiday_list,attendance_device_id from `tabEmployee` where company = %s """ % frappe.db.escape(
		company
	)
...
def get_holiday(holiday_list, date_from,date_to):
