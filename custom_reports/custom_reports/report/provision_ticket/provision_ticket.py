...
def get_data(conditions,filters):
	...
	for emp in conc:
		...
		if filters.get("date_from"):
			...
			ticket_provision_date=getdate(filters.get("date_from"))			
			total_days=frappe.utils.date_diff(date_to,date_from)+1
			openabs=0
			...
			actualworked=total_days-absent
			actual_worked+=actualworked
			year=round(actualworked/365,4)
			#frappe.msgprint(str(actualworked)+'-'+str(year))
			years+=year
			accru=0
			...
			#amount_balance+=bal*currentticketprice
		else:
