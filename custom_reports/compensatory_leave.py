...
from frappe.utils import add_days, cint, date_diff, format_date, getdate,cstr
...
class CompensatoryLeaveRequestCustom(CompensatoryLeaveRequest):
	...
	def validate_holidays(self):
		...
		emp_off=frappe.db.get_value('Employee',{'name':self.employee},['weekly_off','weekly_off_2'],as_dict=1)
		if emp_off:
			date_from=add_days(getdate(self.work_from_date),-1)
			daycount=date_diff(getdate(self.work_end_date),getdate(date_from))
			
			for x in range(daycount):			
				date_from=add_days(getdate(date_from),1)			
				d=add_days(getdate(date_from),1).weekday()
				if wekkday[d]==emp_off.weekly_off or wekkday[d]==emp_off.weekly_off_2:
					if cstr(date_from) not in holidays:
						holidays.append(cstr(date_from))
